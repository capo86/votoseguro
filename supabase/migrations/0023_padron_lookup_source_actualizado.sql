do $$
begin
  if to_regclass('public.staging_regciv_2026_source_next') is null then
    raise exception 'No existe la fuente staging_regciv_2026_source_next.';
  end if;

  if to_regclass('public.staging_regciv_2026_source_previous') is not null then
    raise exception 'La fuente anterior aun existe. Verificar antes de continuar.';
  end if;

  alter table public.staging_regciv_2026_source
    rename to staging_regciv_2026_source_previous;

  alter table public.staging_regciv_2026_source_next
    rename to staging_regciv_2026_source;
end;
$$;
...
as $$
  select
    r.ogc_fid,
    r.cedula,
    coalesce(nullif(trim(actualizado.nombre::text), ''), nullif(trim(r.nombre::text), '')) as nombre,
    coalesce(nullif(trim(actualizado.apellido::text), ''), nullif(trim(r.apellido::text), '')) as apellido,
    nullif(
      trim(
        concat_ws(
          ' ',
          coalesce(nullif(trim(actualizado.nombre::text), ''), nullif(trim(r.nombre::text), '')),
          coalesce(nullif(trim(actualizado.apellido::text), ''), nullif(trim(r.apellido::text), ''))
        )
      ),
      ''
    ) as nombre_apellido,
    ...
    coalesce(actualizado.depart, r.depart) as depart,
    dep.descrip::text as departamento,
    coalesce(actualizado.distrito, r.distrito) as distrito,
    dis.descrip::text as distrito_descripcion,
    coalesce(actualizado.zona, r.zona) as zona,
    zon.descrip::text as zona_descripcion,
    coalesce(actualizado.local, r.local) as local,
  ...
  left join lateral (
    select cedula, depart, distrito, zona, local, mesa, orden, tipo_voto, nombre, apellido
  ...
  left join public.staging_dep as dep
    on dep.depart = coalesce(actualizado.depart, r.depart)
  left join public.staging_dis as dis
    on dis.depart = coalesce(actualizado.depart, r.depart)
   and dis.distrito = coalesce(actualizado.distrito, r.distrito)
  left join public.staging_zon as zon
    on zon.depart = coalesce(actualizado.depart, r.depart)
   and zon.distrito = coalesce(actualizado.distrito, r.distrito)
   and zon.zona = coalesce(actualizado.zona, r.zona)
  left join public.staging_loc as loc
    on loc.dpto = coalesce(actualizado.depart, r.depart)
   and loc.distrito = coalesce(actualizado.distrito, r.distrito)
   and loc.zona = coalesce(actualizado.zona, r.zona)
   and loc.local = coalesce(actualizado.local, r.local)
